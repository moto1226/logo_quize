...
const batches = [
  { grid_id: "logo-regenerate-needed-001", ids: regenerationIds.slice(0, 16) },
  { grid_id: "logo-regenerate-needed-002", ids: regenerationIds.slice(16) }
];
...
function promptFor(batch) {
  ...
  const list = batch.ids
    ...
    .join("\n");
  return [
    "Create a 2048x2048 square image containing a strict 4x4 grid of clean, sharp, recognizable real brand logo marks.",
    "This grid is only for repairing bad logo assets. Do not include any brands except the listed items.",
    ...
    list,
    batch.ids.length < 16 ? `Only fill the first ${batch.ids.length} cells; leave all remaining cells completely blank white.` : "Fill all 16 cells.",
    "",
    "Rules:",
    ...
    "- Text is allowed only if it is part of the official logo itself.",
    "- Do not add unrelated brands.",
    "- Do not crop logos; keep them centered with 15% to 20% padding.",
    "- Avoid gray or black border lines around any cell.",
...
async function ingest(gridId, sourceFile) {
  ...
  if (!batch) throw new Error(`Unknown repair grid: ${gridId}`);
  ...
  for (let index = 0; index < batch.ids.length; index += 1) {
    ...
    fs.writeFileSync(tileWebp, await sharp(tileBuffer).webp({ quality: 88 }).toBuffer());
    fs.writeFileSync(distWebp, await sharp(tileBuffer).webp({ quality: 88 }).toBuffer());
    manifestItems.push({
      index,
      brand_id: brandId,
  ...
  fs.writeFileSync(path.join(repairDataDir, `${gridId}.json`), `${JSON.stringify({
    grid_id: gridId,
    mode: "needed logo regeneration grid",
...
async function contactSheet() {
  const items = regenerationIds.filter((id) => fs.existsSync(path.join(distDir, `${id}.webp`)));
  const cell = 170;
  const label = 40;
  const cols = 5;
  const composites = [];
  ...
  }).composite(composites).webp({ quality: 90 }).toFile(path.join(reviewDir, "logo-regeneration-needed-preview.webp"));
}

async function main() {
  const command = process.argv[2] || "prompts";
  if (command === "prompts") {
    console.log(writePrompts().map((file) => path.relative(root, file).replace(/\\/g, "/")).join("\n"));
    return;
  }
  if (command === "ingest") {
    ...
    console.log(`Ingested ${gridId}: ${await ingest(gridId, source)} tiles`);
    return;
  }
